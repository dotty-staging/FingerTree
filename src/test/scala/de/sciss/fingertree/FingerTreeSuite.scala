...
import scala.collection.immutable.{Vector => Vec}  // until https://issues.scala-lang.org/browse/SI-6150 is fixed :-/
 ...
 */
class FingerTreeSuite extends AnyFeatureSpec with GivenWhenThen {
  val SEED = 0L
  val N1   = 100000
  val N2   = 1000
  ...
  def createStructures: (IndexedSummedSeq[Int, Long], Vec[Int]) = {
    var finger  = IndexedSummedSeq.emptyIntLong
    var scala   = Vec.empty[Int]
    for (i <- 0 until N1) {
